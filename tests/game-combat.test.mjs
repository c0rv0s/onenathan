...
const sounds=[];
const game=await createGame({scene:new THREE.Scene(),camera,blocks:[{x:100,y:-.5,z:0,w:400,h:1,d:100}],places,islandAt:x=>({id:x<50?'studio':x<150?'ios':'games'}),onProgress(){},onSound:event=>sounds.push(event),onSwordHit(){portalChecks++;return portalContact;}});
game.enter(places[0]);
...
assert(sounds.includes('player-hit'),'damage plays player hit cue');
const soundCountBeforePause=sounds.length;
...
assert.equal(sounds.length,soundCountBeforePause,'paused combat stays silent');
...
assert(sounds.includes('enemy-hit'),'sword contact plays hit cue');
assert.equal(sounds.filter(s=>s==='enemy-death').length,game.snapshot().enemies.filter(e=>e.hp===0).length,'each defeated enemy plays one death cue');
const deathSoundCount=sounds.filter(s=>s==='enemy-death').length;
...
assert.equal(sounds.filter(s=>s==='enemy-death').length,deathSoundCount,'fallen bodies do not repeat death cues');
...
const checksBefore=portalChecks,soundsBeforePortal=sounds.length;
...
assert.equal(sounds.length,soundsBeforePortal,'portal contact does not play combat sounds');
portalContact=false;game.exit();game.enter({...places[0],h:-50,x:1000});
game.update(1/60,new Set(),false);
assert.equal(sounds.at(-1),'player-death','falling plays the player death cue');
const soundsAfterFall=sounds.length;
game.exit();game.attack();game.update(1/60,new Set(),false);
assert.equal(sounds.length,soundsAfterFall,'inactive game stays silent');
console.log('Combat audio events, single death cues, and pause/exit silence passed.');
